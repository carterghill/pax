    ...
    match target_os.as_str() {
        "windows" => {
            ...
            let cuda_home = PathBuf::from(
                env::var("CUDA_PATH")
                    .unwrap_or_else(|_| "C:\\Program Files\\NVIDIA GPU Computing Toolkit\\CUDA\\v13.2".into())
            );
            ...
            if cuda_include_dir.join("cuda.h").exists() {
                ...
                builder
                    ...
                    // NVENC encoder only — NVDEC decoder files are omitted because
                    // nvcuvid.lib ships with the Video Codec SDK (separate download),
                    // and WebRTC already has software decoders for H264/H265.
                    ...
                    // NvCodec's Logger.h includes <winsock.h> (WinSock 1), but
                    // webrtc headers use <winsock2.h> (WinSock 2).  Pre-defining
                    // _WINSOCKAPI_ makes winsock.h's include guard skip its body,
                    // avoiding struct/function redefinition errors.  winsock2.h
                    // from webrtc provides all the same types.
                    .define("_WINSOCKAPI_", None);
            } else {
                println!("cargo:warning=CUDA not found at {}; building without NVENC hardware encoding", cuda_home.display());
            }
            // ─────────────────────────────────────────────────────────
